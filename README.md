# ⚖️ Intelligent Contract Risk Analysis & Agentic Legal Assistance System
...
- [Agentic Pipeline — Deep Dive](#-agentic-pipeline--deep-dive)
...
- ♾️ Unlimited or one-sided liability clauses
- ⚖️ Unfair indemnification terms
...
| ✂️ **Clause Segmentation** | Automatically splits contracts into logical clauses using regex boundaries |
...
| ☁️ **Cloud LLM with 3-Level Fallback** | OpenRouter Key 1 → OpenRouter Key 2 → Groq (never fails) |
| 🖥️ **Offline Local LLM** | Runs fully on-device via Ollama (no internet after model download) |
...
> ⚠️ **Security:** The `.env` file is listed in `.gitignore` and must never be committed to version control.
...
- 🎚️ **Risk Sensitivity Slider**: Adjust the ML confidence threshold (0.0 – 1.0)
